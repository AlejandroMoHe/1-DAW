...
def f2a(jugadores: list) -> Jugador:
    mas_partidos = max(jugadores, key=lambda jugador: jugador.partidos_jugados)
    return mas_partidos.nombre

def f2b(jugadores: list) -> Jugador:
    mas_goles = max(jugadores, key=lambda jugador: jugador.goles)
    return mas_goles.nombre
...
def f2f(jugadores: list) -> list:
    ...
    for jugador in jugadores:
        if jugador.goles == 0 and jugador.posicion != "Portero":
            no_goles.append(jugador.nombre)
    return no_goles
...
print(f"2a. Jugador con más partidos: {f2a(atletico_madrid)}")
espacio()
print(f"2b. Jugador con mejor ratio goleador: {f2b(atletico_madrid)}")
espacio()
print(f"2c. Defensa con más goles: {f2c(atletico_madrid)}")
espacio()
print(f"2d. Los jugadores por posición, con mejor ratio goleador son: {f2d(atletico_madrid)}")
espacio()
print(f"2e. Los jugadores con 28 años o menos son: {f2e(atletico_madrid)}")
espacio()
print(f"2f. Los jugadores que nunca han metido un gol y no son porteros: {f2f(atletico_madrid)}")
espacio()
print(f"2g. Los jugadores llevan en el equipo desde antes del 2015 son: {f2g(atletico_madrid)}")
espacio()
print(f"2h. Los centrocampistas que han marcado más goles que algún delantero son: {f2h(atletico_madrid)}")